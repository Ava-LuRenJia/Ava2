...
#define MAX_VARIABLES 100 // 定义一个最大变量数量
...
int main() {
    ...
    int var_count = 0; // 变量计数器

    cpu_init(&cpu);
    init_biu(&biu);

    printf("请输入汇编指令，每行一条，输入 'END' 结束输入:\n");
    ...
    // 执行指令
    char instruction[50]; // 存储从队列中取出的指令
    int instruction_count = 0; // 统计指令数量
    while (dequeue_instruction(&biu, instruction)) {
        execute_code(&cpu, &biu, variables, var_count); // 执行单条指令
        printf("已执行指令: %s\n", instruction);
        instruction_count++;
    }
