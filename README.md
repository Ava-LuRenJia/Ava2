...
**期末大作业**  
这是我自定的题目：  
实现一个基于 8086 CPU 和 8255 外设的智能照明控制系统。  
系统应能够采集光传感器的数据，计算光强，依据设定的逻辑控制灯的状态，并显示当前的光强和灯的状态。使用8086CPU、2片8255芯片、光传感器，其中一片8255连接8086低位，一片连接8086高位。输入先前设定好的汇编语言程序，经过cpu解析后将对应数据传入到8255中，再由8255生成光强度并传输到传感器中，让灯依据光强度数。  
**程序设计**  
输入汇编指令解析  
光照强度读取与控制  
tips:input.txt的内容是示例输入。  
**---------------------------------------------------------------------------------------------------------------------------------------------------------------**  
